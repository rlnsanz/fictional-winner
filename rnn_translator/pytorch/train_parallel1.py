...
def main(partition, device_id):
    ...
    if not flor.is_initialized():
        ...
        flor.writer.Writer.store_load = flor.writer.Writer.partitioned_store_load[2 * predecessor_epoch] + \
                                        flor.writer.Writer.partitioned_store_load[2 * predecessor_epoch + 1]
    ...
    fprint(f'Training optimizer config: {opt_config}')
    ...
    fprint(f'Training LR schedule config: {scheduler_config}')
    ...
    if predecessor_epoch >= 0:
        fprint("Initializing flor predecessor")
